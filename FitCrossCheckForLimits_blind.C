...
namespace LimitCrossCheck{  
  ...
  bool doBlind(true);			    //Ale -- settings for blinded analysis, do not write mu values, do not plot after fit distro with data
        ...
        while( (comp = (RooProduct*) funcIter.Next()) ) {
          ...
          for(int b=1; b<hnominal->GetNbinsX()+1; b++) { // no over/under-flow
            if(hnominal->GetBinContent(b)>0) {
              ...
              //totUp += pow((hm1sigma->GetBinContent(b)-hnominal->GetBinContent(b))/hnominal->GetBinContent(b),2); //Ale
              totDn += pow((hm1sigma->GetBinContent(b)-hnominal->GetBinContent(b))/hnominal->GetBinContent(b),2); //Ale
            }
          }
	  totUp = sqrt(totUp); //Ale
	  totDn = sqrt(totDn); //Ale	
  ...
  void PlotHistosAfterFitEachSubChannel(bool IsConditionnal, double mu){
	...
	NuisParamValue[2*NparNotStat-1-i] = i;
      ...
      c1->cd(2); 
      if(!doBlind) text.DrawLatex( 0.87,0.81, WritDownMuValue );  //Ale 
	...
	if (Ntemp!=0){
      ...
      c2->cd();
      frame->Draw();
      if(!doBlind) text.DrawLatex( 0.84,0.81, WritDownMuValue ); //Ale
  ...
  void PlotHistosAfterFitGlobal(bool IsConditionnal, double mu){
	...
	pdfmodel2->plotOn(frame,LineWidth(0),Components(*comp2),LineColor(0), LineStyle(3), Normalization(Ntemp,RooAbsReal::NumEvent),Name("NoStacked_BkgBeforeFit_"+compname));
      ...
      c2->cd();
      frame->Draw();
      if(!doBlind) text.DrawLatex( 0.73,0.81, WritDownMuValue );  //Ale
      TString ts_chi2 = Form("#chi^{2}=%1.1f", chi2 );
      text.DrawLatex( 0.22, 0.83, ts_chi2 );
